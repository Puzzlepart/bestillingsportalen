...
export const useProvisionDrawer = () => {
  ...
  const onSave = async (): Promise<boolean> => {
    ...
    const hubUrl = targetHubSiteUrl || (isParentMode ? parentSite.HubSiteUrl : currentHubUrl)
    if (hubUrl) {
      ...
      const { taxonomyUpdates } = await applyProjectPropertiesFromMetadata(
        ...
        hubUrl,
        context.props.provisionService
      )
      const hasMetadata =
        Object.keys(properties).length > baseKeyCount || taxonomyUpdates.length > 0

      if (isParentMode || hasMetadata) {
        const added = (await context.props.provisionService.addProjectData(properties, hubUrl)) as
          | { Id?: number }
          | undefined
    ...
    return await context.props.provisionService.addProvisionRequests(
      requestItem,
