...
export function useEditableColumn(
  ...
  useEffect(() => {
    const fetchHubSiteTitle = async () => {
      if (props.parentMode && props.provisionService) {
        try {
          const hubInfo = await props.provisionService.getCurrentHubSite()
          if (hubInfo?.title) {
  ...
  const transformValue = useCallback(
    ...
    [state.teamTemplates, props.provisionService, props.provisionUrl, props.expirationDateMode]
  )
  ...
  useEffect(() => {
    ...
    const resolveHub = async () => {
      ...
      if (
        defaultHub &&
        defaultHub !== currentHubSite &&
        props.provisionService?.resolveHubSiteById
      ) {
        try {
          const hubInfo = await props.provisionService.resolveHubSiteById(defaultHub)
          if (hubInfo) {
