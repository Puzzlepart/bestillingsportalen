...
import { PROJECT_DATA_LIST_TITLE, ProvisionService } from '../../services/ProvisionService'
...
export const applyProjectPropertiesFromMetadata = async (
  ...
  if (!provisionService) return { properties, taxonomyUpdates }

  const list = provisionService.web(hubUrl).lists.getByTitle(PROJECT_DATA_LIST_TITLE)
  ...
  for (const { internalName, value } of entries) {
    ...
    try {
      switch (field.TypeAsString) {
        ...
        case 'User':
        case 'UserMulti': {
          const ids = await ensureUserIds(provisionService.web(hubUrl), value)
...
export const applyTaxonomyUpdatesAfterAdd = async (
  ...
  if (!updates.length || !itemId || !provisionService) return

  try {
    const result = await provisionService
      .web(hubUrl)
      .lists.getByTitle(PROJECT_DATA_LIST_TITLE)
