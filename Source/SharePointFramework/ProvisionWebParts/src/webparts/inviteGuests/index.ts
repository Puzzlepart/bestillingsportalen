...
export default class InviteGuestsWebPart extends BaseClientSideWebPart<IInviteGuestsWebPartProps> {
  ...
  protected onInit(): Promise<void> {
    const targetSiteUrl = (this.properties.guestRequestSiteUrl || '').trim() || this.context.pageContext.web.absoluteUrl;
    const sp: SPFI = spfi(targetSiteUrl).using(SPFx(this.context));
    this._service = new GuestRequestService(sp, this.properties.guestRequestListTitle || 'Guest Requests');
