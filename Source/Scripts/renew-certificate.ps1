...
# --- 2) Variabler ----------------------------------------------------------
# $appId        = Application (client) ID for app-registreringen "Bestillingsportalen" i Entra
# $keyVaultName = Navn på Key Vault hvor sertifikatet ligger (se Bestillingsportalen-ressursgruppen)
# $certName     = Navn på sertifikatet i Key Vault (se Bestillingsportalen-ressursgruppen)
$currDate     = Get-Date
$certEndDate  = $currDate.AddDays(900) | Get-Date -Format 'yyyy/MM/dd'
$appId        = "12345678-90ab-cdef-1234-567890abcdef"
$keyVaultName = "kv-12345-bp"
$certName     = "cert-12345-bestillingsportalen"

# --- 3) Forny sertifikatet -------------------------------------------------
