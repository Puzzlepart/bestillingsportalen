...
- Tjenestekonto for sensitivitetsmerke-funksjonalitet (anvendelse av sensitivitetsmerker), hvis du vil bruke funksjonaliteten. Kan være samme konto som over, men kontoen må være **uten MFA** grunnet en begrensning i Microsoft Graph: `assignedLabels` på grupper og team kan ikke oppdateres med application permissions (re-verifisert august 2026 mot [group-update](https://learn.microsoft.com/en-us/graph/api/group-update)). Løsningen forsøker først en app-only-vei og bruker tjenestekontoen bare hvis den ikke virker — se [Sensitivitetsmerker](./Sensitivity-labels.md) for hvordan du sjekker om kontoen faktisk trengs i din tenant.
