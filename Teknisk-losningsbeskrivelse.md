...
graph TD
    ...
    D --> E(Entra ID App)
    E <--> |Secret/sertifikat| F(Azure Key Vault)
    D --> |SharePoint Site| H[SharePoint REST API]
    D --> |Microsoft 365 Group / Team| I(Microsoft Graph)
    D --> |Viva Engage Community| I
...
- **Client ID** og **client secret** for Entra ID-appen lagres i en dedikert Azure Key Vault og hentes av Logic Apps ved kjøring (input/output skjules i kjørehistorikken).
- Konfigurasjon som ikke kan gjøres via Graph API utføres av runbooken `ConfigureSpace` i Azure Automation, som autentiserer med kontoens **systemtildelte managed identity** og PnP PowerShell.
...
Installasjonen utføres av to PowerShell-skript fra en administrators arbeidsstasjon: `createentraidapp.ps1` (oppretter Entra ID-appen) og `deploy.ps1` (alt annet). Følgende komponenter etableres:
...
| Client secret | `deploy.ps1` genererer en client secret for appen. Standard gyldighet er **1 år** – se [Fornye App Secret](./Refreshing-app-secret.md) for fornyelsesprosessen. |
| Sertifikat | Et sertifikat (self-signed generert av skriptet, eller eget) legges på app-registreringen og i Key Vault. Brukes til klientsertifikat-autentisering mot Microsoft Graph og SharePoint REST API fra Logic Apps. Standard gyldighet 365 dager (`certValidityDays`). |
...
| Azure Automation-konto | `bestillingsportalen-auto` (Free SKU) med **systemtildelt managed identity**. Inneholder PowerShell 7.2-modulene `Az.Accounts` og `PnP.PowerShell`, runbookene `ConfigureSpace` (etterkonfigurasjon av provisjonerte områder) og `GetSiteTemplates`, samt variabler (`tenantId`, `logoUrl` og kryptert `appSecret`). |
| Logic Apps (8 stk.) | `ProcessProvisionRequest` (hovedmotor – provisjonerer godkjente bestillinger), `CheckSiteExists` (sjekker om område/URL finnes, inkl. papirkurv), `ProcessGuests` (inviterer gjestebrukere), `GetHubSites`, `GetSiteTemplates`, `GetTeamsTemplates`, `SyncGroupSettings` og `SyncLabels` (synkroniserer hhv. hub-områder, site-maler, Teams-maler, gruppeinnstillinger og sensitivitetsmerker fra tenanten til SharePoint-listene; kjører ukentlig som standard). |
| API-tilkoblinger (6 stk.) | `bestillingsportalen-spo` (SharePoint Online), `bestillingsportalen-o365` (Office 365 Outlook), `bestillingsportalen-o365users` (Office 365 Users), `bestillingsportalen-teams` (Microsoft Teams), `bestillingsportalen-automation` (Azure Automation) og `bestillingsportalen-kv` (Key Vault). De fire førstnevnte autoriseres manuelt med tjenestekontoen etter installasjon. |
...
### 2.5 Klient (SPFx webdel / Teams-app)

Brukergrensesnittet er en SPFx-basert webdel og Teams-app som leses/skrives mot SharePoint-listene i brukerens egen sikkerhetskontekst. SPFx-pakken leveres og distribueres separat (via tenantens App Catalog og ev. synkronisering til Teams) og inngår ikke i installasjonsskriptene i dette repoet.
...
| **SharePoint Administrator** | Opprette og konfigurere SharePoint-området under `deploy.ps1`. | Samme konto som over (kontoen som kjører `deploy.ps1` bør også være Power Platform- og Teams-administrator). |
...
### 3.4 Arbeidsstasjon og nettverk
...
Runbooken `ConfigureSpace` autentiserer med Automation-kontoens systemtildelte managed identity (PnP PowerShell `-ManagedIdentity`) og utfører etterkonfigurasjon Graph API ikke dekker (PnP-maler, temaer, hub-tilknytning, tilgangsgrupper m.m.):
...
| App-sertifikat (Graph/SPO REST-autentisering) | Key Vault og app-registreringen | 365 dager (`certValidityDays`) | Generer/last opp nytt sertifikat, se `Source/Scripts/renew-certificate.ps1`. |
...
- [Arkitektur](./Architecture.md) – arkitekturdiagram
